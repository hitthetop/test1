...
func InsertTopic(content string,topicName string)bool{
	...
	_,err :=dao.DB.Exec(sqlstr,content,topicName,freshtoken.UserName)
	if err != nil{
		fmt.Println(err)
		return false
	}
	...
	K:=string(rs[:])
	function.WriteWithIoutil("topicName.txt",K)
	fmt.Print("加入成功")
	return true
}
...
func ShowTopic(topicName string) string {
	...
	rowOBJ:= dao.DB.QueryRow(sqlStr,topicName)
...
func InsertComment(topicName string,message string) string {
	...
	sql = fmt.Sprintf(str,topicName)
	_,err =dao.DB.Exec(sql,freshtoken.UserName,message,T)
	if err != nil{
		fmt.Println(err)
		return ""
	}
...
func ChangeTopicName(name string,content string,title string) string {
  ...
  _,err :=dao.DB.Exec(str,content,content,title)
	if err != nil{
		fmt.Println(err)
		return ""
	}
